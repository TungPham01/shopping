...
//    }
    ...
    public  function menuRecusive($parentIdMenuEdit = null, $parentId = 0, $subMark = '') {
        $data = Menu::where('parent_id',$parentId)->get();

        foreach($data as $dataItem){
            if($parentIdMenuEdit == $dataItem->id && !is_null($parentIdMenuEdit)){
                $this->html .= '<option selected value="'. $dataItem->id . '">'. $subMark .$dataItem->name .'</option>';
            }else{
                $this->html .= '<option value="'. $dataItem->id . '">'. $subMark .$dataItem->name .'</option>';
            }
            $this->menuRecusive($parentIdMenuEdit, $dataItem->id,  $subMark.'--' );
        }
        return $this->html;
    }
}